...
/*
 ...
 2.hashCode数字过大，或许会经过取余生成一个较小的数字，比如是对999进行取余，那么得到的结果始终处于0～999之间。但是，这样做的弊端在于取余所得到的值，可能是相同的，这样可能导致完全不相干的键值对（取余后值key相等）而覆盖。于是出现了数组中套链表实现的数组。这样，key取余得到的值相等的键值对，都将保存在同一个链表数组中，当查找key所对应的值时，首先获取到该链表数组，然后遍历该数组，取出正确的key所对应的值即可。
 3.当相同hashCode的链表数组的深度较长时，会造成查找o(n)的效率问题，后期可通过二叉平衡数解决
 */
